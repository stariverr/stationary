...
export const LibraryListQuerySchema = z.object({
    ...
    count: z.preprocess((val) => (val === "" || val === undefined ? undefined : Number(val)), z.number().int().positive().gte(10).lte(100).optional()),
    keyword: z.string().optional(),
});

export const LibraryCreateBodySchema = z.object({
    name: z.string().min(1, "Name is required"),
    description: z.string().optional().default(""),
}).strict();

export const LibraryUpdateBodySchema = z.object({
    id: z.uuid(),
    name: z.string().min(1, "Name is required").optional(),
    description: z.string().optional(),
}).strict();

export const LibraryMoveItemsBodySchema = z.object({
...
export const toLibraryCreateValues = (
    body: z.infer<typeof LibraryCreateBodySchema>,
    ownerId: string,
...
router.get("/list", requireAuth, validator("query", (value, c) => {
    ...
    const totalResult = await db.select({ total: count() }).from(Library).where(and(...where));
...
router.post("/create", requireAuth, validator("json", (value, c) => {
    ...
    const newLib = await db.insert(Library).values(toLibraryCreateValues(body, user.id)).returning();
    return c.json(success(Code.SUCCESS, newLib[0]));
});

router.post("/update", requireAuth, validator("json", (value, c) => {
...
router.post("/delete/:id", requireAuth, async (c) => {
    ...
    if (deleted.length === 0) return c.json(error(Code.NOT_FOUND, "Library not found"));
    return c.json(success(Code.SUCCESS, deleted[0]));
});

router.post("/move-items", requireAuth, validator("json", (value, c) => {
    ...
    const targetLibraries = await db
        ...
        .from(Library)
        .where(eq(Library.id, body.target_library_id))
        .limit(1);
    ...
    const selectedPosts = body.post_ids.length > 0
        ? await db
            .select({ id: Post.id })
            .from(Post)
    ...
    const selectedMedia = body.media_ids.length > 0
        ? await db
            .select({ id: Media.id, post_id: Media.post_id })
            .from(Media)
            .where(inArray(Media.id, body.media_ids))
        : [];
