...
import { Library, Post, Media } from '@/db/schema';
import { eq, isNull } from 'drizzle-orm';

async function migrate() {
    console.log('Starting Library Migration...');

    let defaultLib = await db.select().from(Library).where(eq(Library.name, 'Default Library')).limit(1).then(rows => rows[0]);

    if (!defaultLib) {
        console.log('- Creating Default Library...');
        const newLibs = await db.insert(Library).values({
