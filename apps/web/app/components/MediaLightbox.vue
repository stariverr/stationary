...
import { X, ChevronLeft, ChevronRight, Info, Layers, Link as LinkIcon, FileImage } from '@lucide/vue';
    ...
    if (displayMode.value === 'stacked' && selectedMedia.value?.post_id) {
        ...
        try {
            ...
            if (response && response.success && response.data) {
                ...
                postSiblings.value = (response.data.media || []).map((m: any) => {
                    return {
                        ...m,
                        url: cdnPrefix(m.primary_file_path),
                        type: m.type
                    };
                });
            } else {
        ...
        postSiblings.value = medias.value;
        const index = postSiblings.value.findIndex(m => m.id === newId);
        currentIndex.value = Math.max(0, index);
        nextTick(() => {
            if (swiperInstance.value) swiperInstance.value.slideTo(currentIndex.value, 0);
            ...
            <div v-if="isVisible" class="fixed inset-0 z-[200] flex bg-black/95 backdrop-blur-sm pointer-events-auto">
                ...
                <div class="flex-1 relative flex items-center justify-center group/lightbox min-w-0">
                    ...
                    <div class="w-full h-full">
                        <swiper :slides-per-view="1" :loop="false" :initial-slide="currentIndex" @swiper="onSwiper" @slideChange="onSlideChange" class="w-full h-full">
                            <swiper-slide v-for="(media, index) in postSiblings" :key="media.id || index" class="flex items-center justify-center">
                                <div class="w-full h-full flex items-center justify-center p-4 md:p-12">
                                    <VideoPlayer v-if="media.type?.toLowerCase() === 'video'" :src="media.url || media.media_url" class="max-h-full max-w-full h-full w-auto drop-shadow-2xl rounded-sm" />
                                    <img v-else :src="media.url || media.media_url" class="max-h-full max-w-full object-contain drop-shadow-2xl rounded-sm" />
                                </div>
                            </swiper-slide>
                        </swiper>
                    </div>
                ...
                <div class="w-[320px] bg-white h-full shrink-0 flex flex-col shadow-2xl relative z-[210] overflow-y-auto hidden md:flex">
                    <div class="p-5 border-b border-gray-100 bg-gray-50/50">
                        <h3 class="font-semibold text-gray-900 flex items-center gap-2">
                            <Info class="w-4 h-4 text-blue-500" /> {{ $t('common.details', 'Asset Details') }}
                        </h3>
                    </div>
                    ...
                    <div class="p-5 space-y-6 flex-1">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <div class="text-sm font-medium text-gray-900 break-words">{{ currentMediaItem?.title || 'Untitled Asset' }}</div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <div class="text-sm text-gray-700">{{ currentMediaItem?.source || 'Local' }}</div>
                                </div>
                            </div>
                            
                            <div>
                                <label class="text-[10px] uppercase text-gray-400 font-bold tracking-wider mb-1 block">Published Time</label>
                                <div class="text-sm text-gray-700">{{ formatDate(currentPublishedTime) }}</div>
                            </div>
                        </div>
                        ...
                        <div v-if="postDetail" class="space-y-3">
                            ...
                            <div class="bg-blue-50/50 border border-blue-100 rounded-xl p-4 transition-all hover:bg-blue-50 cursor-pointer" @click="navigateTo(`/posts/${postDetail.id}`)">
                                <div class="flex items-center gap-2 mb-2">
                                    <div class="w-6 h-6 rounded-full bg-white shadow-sm flex items-center justify-center text-xs font-bold text-blue-600">
                                        {{ postDetail.author?.charAt(0) || 'A' }}
                                    </div>
                                    <span class="text-sm font-semibold text-gray-900">{{ postDetail.author || 'Unknown Author' }}</span>
                                </div>
