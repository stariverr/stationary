...
export const useLibraryStore = defineStore('library', () => {
    ...
    const { data: librariesData, isLoading, refetch } = useQuery({
        queryKey: ['libraries'],
        queryFn: async () => {
            const response = await useApi<{ success: boolean; data: { list: LibraryItem[]; total?: number } }>('/library/list');
            if (response && response.success && response.data) {
                // If there's no active library selected, set it to the first one
                const firstLibrary = response.data.list[0];
                if (!activeLibraryId.value && firstLibrary) {
