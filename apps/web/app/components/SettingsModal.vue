                ...
                <CardContent class="grid gap-6">
                  <div class="flex items-center justify-between">
                    <div class="space-y-0.5">
                      ...
                      <p class="text-sm text-muted-foreground">{{ $t('settings.general.expand_detail_desc') }}</p>
                    </div>
                  ...
                  <div class="grid gap-3">
                    <div class="space-y-0.5">
                      ...
                      <p class="text-sm text-muted-foreground">{{ $t('settings.general.language_desc') }}</p>
                    </div>
              ...
              <template v-else>
                ...
                <Card>
                  ...
                  <CardContent>
                    ...
                    <div v-else-if="apiTokens.length === 0" class="text-center py-10 text-sm text-muted-foreground border border-dashed rounded-xl bg-gray-50/20">
                      <Key class="w-8 h-8 text-gray-300 mx-auto mb-2" />
                      <p>{{ $t('settings.api_keys.empty') }}</p>
                    </div>

                    <div v-else class="divide-y">
                      <div v-for="token in apiTokens" :key="token.id" class="flex items-center justify-between py-4 first:pt-0 last:pb-0">
                        <div class="space-y-1 min-w-0 flex-1 pr-4">
                          <div class="flex items-center gap-2">
                            <span class="font-bold text-sm text-gray-900 truncate">{{ token.name }}</span>
                            <span class="px-2 py-0.5 rounded text-[10px] font-mono font-medium bg-gray-100 border text-gray-600">
                          ...
                          <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                            ...
                            <span class="flex items-center gap-1">
                              ...
                              {{ $t('settings.api_keys.expires_label', { date: formatDate(token.expires_at) }) }}
                            </span>
                            <span v-if="token.last_used_at">
                              {{ $t('settings.api_keys.last_used_label', { date: formatDate(token.last_used_at) }) }}
                            </span>
                            <span v-else>
