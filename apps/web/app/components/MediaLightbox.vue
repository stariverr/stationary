        ...
        try {
            ...
            if (response && response.success && response.data) {
                ...
                postSiblings.value = (response.data.media || [])
                    ...
                    .filter((m): m is MappedSiblingMedia => !!m);
            } else {
                            ...
                            <swiper-slide v-for="(media, index) in postSiblings" :key="media.id || index"
                                ...
                                <div class="w-full h-full flex items-center justify-center p-4 md:p-12">
                                    <VideoPlayer v-if="media.type?.toLowerCase() === 'video'"
                                        :src="media.url || media.media_url || ''" :subtitles="media.subtitles"
                                        :width="media.width" :height="media.height"
                                        class="max-h-full max-w-full h-full w-auto drop-shadow-2xl rounded-sm" />
                                    <HeicImage v-else :src="media.url || media.media_url || ''"
                    ...
                    <div v-if="displayMode === 'stacked' && postSiblings.length > 1"
                        ...
                        <div v-for="(media, index) in postSiblings" :key="media.id || index"
                            ...
                            :class="index === currentIndex
                                ...
                                ">
                            <HeicImage v-if="media.type?.toLowerCase() !== 'video'" :src="getOptimizedImageUrl(media.url || media.media_url || '', {
                                width: 320,
                                height: 240,
                                fit: 'cover',
                                gravity: 'auto',
                            })
                                " :mime-type="media.mime_type || media.mimeType || undefined" class="w-full h-full object-cover" />
                            <div v-else
                    ...
                    <div class="p-5 space-y-6 flex-1">
                        ...
                        <div v-if="isVideo" class="space-y-3">
                            <label
                             ...
                             <button :disabled="!currentMediaItem || regeneratingCoverMediaId === currentMediaItem.id"
                                 ...
                                 <Loader2 v-if="currentMediaItem && regeneratingCoverMediaId === currentMediaItem.id"
