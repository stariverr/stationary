...
export const AiEnrichmentService = {
    ...
    async enrichPost(postId: string): Promise<void> {
        ...
        console.log(
            `[AI ENRICH] Enriching post ${postId} containing ${mediaList.length} active media items.`,
        );
    ...
    async enrichMediaItem(
        ...
        if (!aiService) {
            console.log(
                `[AI ENRICH] AI is not configured for library ${media.library_id}. Skipping.`,
            );
            return;
        }
        ...
        if (existingDoc) {
            console.log(
                `[AI ENRICH] Media item ${media.id} is already enriched and up-to-date (hash match). Skipping.`,
            );
            return;
        }
        ...
        if (!downloadResponse.ok) {
            throw new Error(
                `Failed to download physical file from S3: ${downloadResponse.statusText}`,
            );
        }
        ...
        if (storageIsPublic && cdnUrl) {
            ...
            console.log(
                `[AI ENRICH] Fallback to buffer. storageIsPublic: ${storageIsPublic}, cdnUrl: ${cdnUrl}`,
            );
        }
