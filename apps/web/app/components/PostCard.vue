...
<template>
    <ContextMenu>
        <ContextMenuTrigger as-child>
            <div class="group relative flex flex-col gap-2 p-2 rounded-xl transition-all duration-200 cursor-pointer"
                ...
                <div class="relative aspect-4/3 rounded-lg overflow-hidden border border-gray-100 bg-gray-50">
                    ...
                    <!-- Video Card -->
                    <div v-else-if="post.type === 'MULTI_MEDIA' && post.media?.[0]?.type === 'VIDEO'"
                        class="w-full h-full relative bg-gray-900">
                        <video v-if="isHovered" ref="videoRef" :src="`${post.media?.[0]?.url}#t=0.01`"
                        ...
                        <div v-else-if="!post.media?.[0]?.poster && !isHovered"
        ...
        <ContextMenuContent class="w-52">
            <ContextMenuItem v-if="post.type === 'MULTI_MEDIA' && post.media?.[0]?.type === 'VIDEO'"
