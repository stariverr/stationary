...
import { CheckSquare, ChevronLeft, ChevronRight, LayoutGrid, Menu, Search, X, MoreHorizontal, Layers } from '@lucide/vue';
...
const { medias, isLoadingMedia, keyword, source, displayMode, total, page, count, selectedMediaId } = storeToRefs(store);
...
const movableVisibleMediaIds = computed(() => ((medias.value as any[]) || []).filter((media) => !media.post_id).map((media) => String(media.id)));
const areAllMovableVisibleMediaSelected = computed(() => movableVisibleMediaIds.value.length > 0 && movableVisibleMediaIds.value.every((id) => selectedMediaIds.value.has(id)));
...
<template>
    <div class="flex-1 min-h-0 flex flex-col bg-white overflow-hidden relative">
        ...
        <div class="h-14 border-b border-gray-100 flex items-center justify-between px-3 sm:px-6 shrink-0">
            ...
            <div class="flex-1 max-w-2xl px-1 sm:px-4 flex items-center gap-1.5 sm:gap-3">
                ...
                <div class="relative flex-1 group">
                    ...
                    <input v-model="keyword" type="text" :placeholder="$t('common.search', 'Search media...')"
            ...
            <div class="flex items-center gap-1 sm:gap-2 bg-gray-100 p-0.5 sm:p-1 rounded-lg">
                ...
                <!-- Display Mode Toggle -->
                <button @click="displayMode = 'flat'" :class="displayMode === 'flat' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700'" class="p-1 sm:p-1.5 rounded-md transition-all flex items-center gap-1 text-xs sm:text-sm font-medium">
                    <LayoutGrid class="w-3.5 h-3.5 sm:w-4 sm:h-4" /> <span class="hidden sm:inline">Flat</span>
                </button>
                <button @click="displayMode = 'stacked'" :class="displayMode === 'stacked' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700'" class="p-1 sm:p-1.5 rounded-md transition-all flex items-center gap-1 text-xs sm:text-sm font-medium">
                    <Layers class="w-3.5 h-3.5 sm:w-4 sm:h-4" /> <span class="hidden sm:inline">Stacked</span>
                </button>
            </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-6 bg-gray-50/50">
            <div v-if="Array.isArray(medias)" ref="gridContainer"
                class="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-4">
                <MediaCard v-for="(media, index) in medias" :key="media.id" :media="media"
                    :data-index="index"
