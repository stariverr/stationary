...
async function run() {
  ...
  const existingPrimaryMediaIds = await db
    ...
    .where(
      and(
        eq(Track.is_primary, true),
        eq(Track.delete_status, DeleteStatus.ACTIVE),
      ),
    );

  const mediaIdsWithPrimary = new Set(
    existingPrimaryMediaIds
      .map((m) => m.media_id)
  ...
  if (tracksToMarkPrimary.length === 0) {
    console.log(
      "No existing tracks need is_primary migration. All up to date!",
    );
    process.exit(0);
  }

  console.log(
    `Marking ${tracksToMarkPrimary.length} tracks as is_primary = true...`,
  );
