                        ...
                        <div class="grid gap-6">
                                ...
                                <CardContent class="grid gap-6">
                                    <div class="flex items-center justify-between">
                                        <div class="space-y-0.5">
                                            ...
                                            <p class="text-sm text-muted-foreground">{{
                                                $t('settings.general.expand_detail_desc') }}</p>
                                        </div>
                                        <Switch :checked="expandDetailByDefault"
                                    ...
                                    <div class="grid gap-3">
                                        <div class="space-y-0.5">
                                            ...
                                            <p class="text-sm text-muted-foreground">{{
                                                $t('settings.general.language_desc') }}</p>
                                        </div>
                                        <Select :model-value="locale" @update:model-value="handleLocaleChange">
                                            ...
                                            <SelectContent>
                                                <SelectGroup>
                                                    <SelectItem v-for="l in localeOptions" :key="l.code"
                                                        :value="l.code">
                                                        {{ l.name }}
                                                    </SelectItem>
                                                </SelectGroup>
                            ...
                            <Card>
                                ...
                                <CardContent class="py-10 text-center text-sm text-muted-foreground italic">
                                    {{ $t('settings.general.other_empty') }}
                                </CardContent>
                            </Card>
                        </div>
                    </TabsContent>
                            ...
                            <CardContent class="space-y-6">
                                <div v-if="session.data?.user"
                                    ...
                                    <Avatar class="w-16 h-16 border-2 border-white shadow-sm">
                                        <AvatarImage v-if="session.data.user?.image" :src="session.data.user?.image"
                                    ...
                                    <div class="space-y-1">
                                        <h4 class="text-lg font-bold leading-none">{{ session.data.user?.name || 'User'
                                            }}</h4>
                                        <p class="text-sm text-muted-foreground">{{ session.data.user?.email || '' }}
                                        </p>
                                        <div
                            ...
                            <Card v-if="generatedToken" class="border-amber-200 bg-amber-50/20">
                                ...
                                <CardContent class="grid gap-4">
                                    <div class="flex items-center gap-2">
                                        <Input readonly v-model="generatedToken"
                                    ...
                                    <Button class="w-full bg-amber-600 hover:bg-amber-700 text-white mt-2"
                                ...
                                <Card>
                                    ...
                                    <CardContent class="grid gap-4">
                                        <div class="grid gap-2">
                                            ...
                                            <Input id="token-name" v-model="newTokenName"
                                        ...
                                        <div class="grid grid-cols-2 gap-4">
                                                ...
                                                <Select v-model="newTokenLibraryId">
                                                    ...
                                                    <SelectContent>
                                                        <SelectGroup>
                                                            ...
                                                            <SelectItem v-for="lib in libraryStore.libraries"
                                            ...
                                            <div class="grid gap-2">
                                                ...
                                                <Select v-model="newTokenExpiresIn">
                                                    ...
                                                    <SelectContent>
                                                        <SelectGroup>
                                                            <SelectItem value="0">{{ $t('settings.api_keys.exp_never')
                                                                }}</SelectItem>
                                                            <SelectItem value="604800">{{ $t('settings.api_keys.exp_7d')
                                                                }}</SelectItem>
                                                            <SelectItem value="2592000">{{
                                                                $t('settings.api_keys.exp_30d') }}</SelectItem>
                                                            <SelectItem value="7776000">{{
                                                                $t('settings.api_keys.exp_90d') }}</SelectItem>
                                                        </SelectGroup>
                                                    </SelectContent>
                                                </Select>
                                            </div>
                                        ...
                                        <Button class="w-full mt-2" :disabled="!newTokenName.trim() || isGenerating"
                                            @click="generateToken">
                                            <Loader2 v-if="isGenerating" class="w-4 h-4 mr-2 animate-spin" />
                                            <Plus v-else class="w-4 h-4 mr-2" />
                                            {{ isGenerating ? $t('settings.api_keys.generating') :
                                ...
                                <Card>
                                    ...
                                    <CardContent>
                                        <div v-if="isLoading"
                                            ...
                                            <p class="text-sm">{{ $t('settings.api_keys.loading') }}</p>
                                        </div>
                                        ...
                                        <div v-else class="divide-y">
                                            <div v-for="token in apiTokens" :key="token.id"
                                                ...
                                                <div class="space-y-1 min-w-0 flex-1 pr-4">
                                                    <div class="flex items-center gap-2">
                                                        ...
                                                        <span
                                                            ...
                                                            {{ token.prefix }}_{{ token.first_four || '' }}...{{
                                                            token.last_four }}
                                                        </span>
                                                    </div>
                                                ...
                                                <Button variant="ghost" size="icon"
                                                    ...
                                                    :disabled="isRevoking === token.id" @click="revokeToken(token.id)">
                                                    <Loader2 v-if="isRevoking === token.id"
